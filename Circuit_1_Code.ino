...
// MQTT Broker
const char* mqtt_server = "192.168.199.85";

// Pin definitions
#define LED_WLAN 13 // D7
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "two/servo") {
    ...
    // servo_function();  // Implement your servo control function here
  ...
  } else if (String(topic) == "one/setpoint") {
    setpoint = message;
    setpointInt = setpoint.toInt();
    Serial.println("Setpoint set to " + message);
    
    while (lightintencity > setpointInt || lightintencity < setpointInt) {
      if (lightintencity > setpointInt) {
        client.publish("one/sub", "Hi Ravindu!");
      } else {
        client.publish("one/add", "Bye Ravindu!");
      }
      delay(1000);
      check_brightness();
      delay(1000);
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("NodeMCUClient")) {
      ...
      client.subscribe("one/mode");
      client.subscribe("one/setpoint");
